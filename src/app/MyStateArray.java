...
Класс с массивами обектов окна отношений Состояния и методы для работы с элементами класса.
...
Он включает в себя:
    ...
    - и др.
Также есть возможность автоматического упорядочивания элементов Состояния, присутствующих в проекте.
Расчет размеров окна для их отображения.
Или использование сохраненных размеров окна Состояния прошлого сеанса(сохранения).
 */
 ...
 * 
 */ 
public class MyStateArray {

    private ArrayList <OntoState> myArray;

    private ArrayList <MyEll> myArrayToIn;
    ...
    * Используется при автоиатическом расположении объектов  разных классов отношений на экране.
    ...
    private void initComponent(){
        this.myArray = new  ArrayList<OntoState>();
        this.myArrayToIn = new ArrayList<MyEll> ();
     ...
     * @param ontoState
     */
    public void addToMyArray(OntoState ontoState){
        this.myArray.add(ontoState);
        //Сформируем размеры окна для отображения объектов отношения.
        //Добавим к текущим размерам окна размеры нового отношения.
        //windowSize.setSize(windowSize.getHeight()+ontoState.getWindowSise().getHeight(), windowSize.getWidth()+ontoState.getWindowSise().getWidth());
    }
    ...
    public void createLinesArrayFromArray(){
        ...
        for(int i=0;i<this.myArray.size();i++){
            OntoState ontoState =this.myArray.get(i);
            //просмотрим все элементы отношения и создадим линию от каждого элемента к классу.
            MyEll klassEll= ontoState.getEllKlass();
            MyEll elemEll=  ontoState.getEllElem();
     ...
     * @param ontoStateAr
     */
    public void addMyArray(ArrayList <OntoState> ontoStateAr){
        for(int i=0;i<ontoStateAr.size();i++){
            addToMyArray(ontoStateAr.get(i));
        }
    }
    /**Получим значение массива отношений.*/
    public ArrayList <OntoState> getMyArray(){
        return this.myArray;
    }
    ...
    public <MyLineState> MyLineArray  getMyLineArray(){
 ...
 * @autor 655 29.06.2016/
     ...
     * Эллипс имеет линии. Значит он из массива ontoStateArray.
       Эллипс не имеет линий. Значит он из массива ontoStateArrayToIn.
    ...
    public void importToPaintEll(ArrayList <OntoState> myAr){
        ...
        for (int ti=0; ti< myAr.size();ti++){
            ...
            //for (int tik=0; tik< ontoStateArray.get(ti).getElemEllArray().getEllArray().size();tik++){
    ...
    /**Найдем эллипс среди элементов отношений ontoStateArray
     * @param text.
     * @return */
   ...
   public void removePaintEllipseTree (MyEll myEll){
    ...
    public void renewMyArrayFromPaint(String pOblast){
        ...
        OntoState ontoState;
        ArrayList <OntoState> ontoStateAr= new ArrayList<OntoState> ();//массив пар отношений.
        //если есть линии
            ...
            for(MyLine myLine:myLineArray.getLineArray()){
                ...
                if(myLine.getPriznakUrlAr().size()>0){
                    for(PriznakUrl priznakUrl: myLine.getPriznakUrlAr()){
                        //строим отношение.
                        ontoState=new OntoState(
                                    this.findPaintEll(myLine.getText2()), 
                                    this.findPaintEll(myLine.getText1()), 
                                    priznakUrl.get_priznak(), 
                                    priznakUrl.get_url(),"Состояния"
                                ,myLine.getTimeDate_MyEllRectState().getMyData()[6][1]
                                ,myLine.getTimeDate_MyEllRectState().getMyData()[7][1]
                                ,myLine.getTimeDate_MyEllRectState().getMyData()[8][1]);
                            //добавим его в массив отношений.
                            ontoStateAr=uniqueMyCompos(ontoStateAr,ontoState);
                        }
                    }else{
                        //строим отношение.
                            ontoState=new OntoState(
     ...
     * @return 
     @autor 655 29.06.2016
     */
    public ArrayList <OntoState> uniqueMyCompos(ArrayList <OntoState> ontoStateAr, OntoState ontoState){
        int count=0;
        for(OntoState ontoState1:ontoStateAr){
            if(ontoState1.getKlass().trim().equals(ontoState.getKlass().trim())
                    &&ontoState1.getElem().trim().equals(ontoState.getElem().trim())
                    &&ontoState1.getPriznak().trim().equals(ontoState.getPriznak().trim())
                    &&ontoState1.getUrl().trim().equals(ontoState.getUrl().trim())){
                count++;
            }
        }
        if(count==0)ontoStateAr.add(ontoState);
        return ontoStateAr;
    }
    
    /**
     * Подсчитаем, сколько отношений с такими парами класс-элемент записано в массиве.*/
    private int countAmountOntoState(ArrayList <OntoState> ontoStateArKlassAr,OntoState ontoState ){
        int count=0;
        for(OntoState ontoState1: ontoStateArKlassAr){
            if(ontoState1.getKlass().equals(ontoState.getKlass())
                    &&ontoState1.getElem().equals(ontoState.getElem())){
                    count++;
     ...
     * @param klass
     * @return */
    public boolean findKlassB(ArrayList <OntoState> ontoStateArr, String klass){
        if(ontoStateArr.size()==0)return false;
        for(OntoState ontoState: ontoStateArr){
            if(ontoState.getKlass().equals(klass)){
            return true;
            }
        }
        return false;
        ...
        if(!treeEll.isEmpty()){
            ...
            while(lineLevelAr.size()>0){
                ...
                for(MyLine line:lineLevelAr){
                    ...
                    for (int i=0;i<this.myLineArray.getLineArray().size();i++){
                        //перебираем все линии массива линий
                        MyLineState ml=(MyLineState) myLineArray.getLineArray().get(i);
                       //переназначаем исходящие линии для перемещаемого эллипса
    ...
    */
    ...
    /***/
    public void addPaintOnto(OntoState ontoState){
        
        this.paintEllArray=unique(this.paintEllArray,ontoState.getEllKlass());//добавим на экран Начальное состояние
        this.paintEllArray=unique(this.paintEllArray,ontoState.getEllElem());//добавим на экран Конечное состояние
        this.myLineArray.setLineArray(uniqueMyLine(this.myLineArray.getLineArray(), 
                new MyLineState(ontoState.getEllElem()
                        ,ontoState.getEllKlass()
                        ,ontoState.getPriznak()
                        ,ontoState.getPriznakSart()
                        ,ontoState.getPriznakEnd()
                        ,ontoState.getPriznakInterval()) 
        ));//добавим линию с Начальным, Конечным состояниями и Действием между ними.
  ...
  /***/
    ...
    public void ellPositionedTree() {
        ...
        for(MyEll myEll: getPaintEll()){
            //Если у эллипса нет Выходящих линий,берем его в массив.
            if(!findLineText1EllText1B(getPaintEll(), myEll.getText())){
                ellTemp.add(myEll);
            }
        }
